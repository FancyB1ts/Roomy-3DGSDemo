...
namespace LanguageLocalization
{
    ...
    public class Localization_SOURCE_Editor : Editor
    {
        ...
        private void OnEnable()
        {
            ...
            categories = new string[l.loadedCategories.Count + 1];
            categories[0] = "All";
            for (int i = 0; i < l.loadedCategories.Count; i++)
                categories[i + 1] = l.loadedCategories[i];
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (l.LoadedLanguageFiles != null &&
                l.LoadedLanguageFiles.Length > 0 &&
                l.selectedLanguage >= 0 &&
                l.selectedLanguage < l.LoadedLanguageFiles.Length &&
                l.LoadedLanguageFiles[l.selectedLanguage] != null)
            {
                PL("Active Language: " + l.LoadedLanguageFiles[l.selectedLanguage].name);
            }
            else
            {
                EditorGUILayout.HelpBox("⚠ No language file assigned or index is out of bounds.", MessageType.Warning);
            }
            PS(5);
            ...
            if (PB("Add Key"))
            {
                ...
                addKey = true;
                l.InternalEditor_RefreshInternalLocalization();
            }

            // Auto-sync button
            if (PB("Auto-Sync All Keys"))
            {
                if (EditorUtility.DisplayDialog("Auto-Sync Confirmation", 
                    "This will replace all current keys with all keys from the Localization Manager. Continue?", 
                    "Yes", "Cancel"))
                {
                    l.AutoSyncAllKeys();
                    l.RefreshTextElementsAndKeys();
                }
            }
            ...
            if (addKey)
            {
                ...
                if (categorySelected)
                {
                    ...
                    if (PB("Add All", GUILayout.Width(120)))
                    {
                        ...
                        {
                            if (Localization_SOURCE_Window.localizationElements[i].categoryIndex != category)
                                continue;
                            l.InternalEditor_AddKey(Localization_SOURCE_Window.localizationElements[i].key);
                        }
                        addKey = false;
                        categorySelected = false;
                        category = 0;
